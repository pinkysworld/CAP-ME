...
| Closed multi-host run records 68/90 acknowledged and 71/90 receiver completions | `results/processed/fso/multihost/manifest.json` | Eight internal-network containers under one frozen declared fault sequence; not a field result |
| Multi-host packet-model phase MAE is 0.0780 and Brier score is 0.0594 | Same | Implementation concordance under the injected rates and delay approximation, not censor calibration |
| Multi-host recovery expires 41 incomplete fragment sets and finishes with zero in-flight fragment or coded-message state | Same | Deadline-bound laboratory cleanup measurement |
...
| The feedback follow-up uses 12 newly frozen seeds and a precommitted paired decision rule | `configs/fso-feedback-source.json`; `configs/fso-feedback-evaluation.json`; future `feedback_audit.json` | Confirmatory only after the config commit; no benefit claim unless its CI rule is met |
